...
var cduLegs = {
  ...
  setAltitude : func(wpIndex, entry) {

  if (entry == nil or entry == "") return;

  # Handle SPD/ALT format (e.g. 250/5000 or 250/FL180)
  if (contains(entry, "/")) {
   ...
   # Speed (knots)
if (spd != "" and isdigit(spd)) {
  setprop(
    "autopilot/route-manager/route/wp["~wpIndex~"]/speed-kts",
    num(spd)
  );
  setprop(
    "autopilot/route-manager/route/wp["~wpIndex~"]/speed-restriction",
    1
  );
}


  # ---- ALTITUDE ONLY (original behavior) ----
  if (substr(entry,0,2) == "FL") {
    setprop(
      "autopilot/route-manager/route/wp["~wpIndex~"]/altitude-ft",
      num(substr(entry,2)) * 100
    );
  }
